    ...
    responseSchema: {
      ...
      properties: {
        name: {
          ...
          description: "A short, fitting name for the theme (1-3 words)",
        },
        type: {
          type: SchemaType.STRING,
          description:
            "A one-word category label (e.g. 'Warrior', 'Beverage', 'Error')",
        },
      },
      required: ["name", "type"],
    },
...
server.tool(
  ...
  async ({ theme }) => {
    const prompt = `Generate a character or entity that fits this theme: "${theme}".
  Return a JSON object with:
  - name: a short, fitting name (1-3 words)
  - type: a one-word category label`;

    const result = await model.generateContent(prompt);
    const props = JSON.parse(result.response.text());

    return widget({
      props,
      output: text(`Generated widget for theme: ${theme}`),
    });
  },
);
...
server.tool(
  {
    name: "detect-client",
    description: "Show info about the connecting client and its capabilities",
    schema: z.object({}),
  },
  async (_params, ctx) => {
